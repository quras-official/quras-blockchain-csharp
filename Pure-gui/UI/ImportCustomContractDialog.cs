...
namespace Pure.UI
{
    internal partial class ImportCustomContractDialog : Form
    {
        ...
        {
            comboBox1.Items.AddRange(Program.CurrentWallet.GetContracts().Where(p => p.IsStandard).Select(p => ((KeyPair)Program.CurrentWallet.GetKey(p.PublicKeyHash)).PublicKey).ToArray());
        }
