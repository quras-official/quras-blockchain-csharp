...
namespace Quras.Cryptography
{
    internal static class CertificateQueryService
    {
        ...
        public static CertificateQueryResult Query(Quras.Cryptography.ECC.ECPoint pubkey)
        {
        ...
        private static void UpdateResultFromFile(UInt160 hash)
        {
            ...
            if (!hash.Equals(Contract.CreateSignatureRedeemScript(Quras.Cryptography.ECC.ECPoint.DecodePoint(cert.PublicKey.EncodedKeyValue.RawData, Quras.Cryptography.ECC.ECCurve.Secp256r1)).ToScriptHash()))
            {
                results[hash].Type = CertificateQueryResultType.Missing;
                return;
            }
