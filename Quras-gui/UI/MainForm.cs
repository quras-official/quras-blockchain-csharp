...
namespace Quras.UI
{
    internal partial class MainForm : Form
    {
                ...
                if (Program.CurrentWallet.WalletHeight <= Blockchain.Default.Height + 1)
                {
                    if (balance_changed)
                    {
                        ...
                        foreach (var asset in assets.Values)
                        {
                            ...
                            {
                                string asset_name = asset.Asset.AssetType == AssetType.GoverningToken ? "Quras" :
                                                    asset.Asset.AssetType == AssetType.UtilityToken ? "QurasGas" :
                                                    asset.Asset.GetName();
        ...
        private void 创建新地址NToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            KeyPair key = (KeyPair)Program.CurrentWallet.CreateKey();
        ...
        private void 查看私钥VToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            KeyPair key = (KeyPair)Program.CurrentWallet.GetKeyByScriptHash(contract.ScriptHash);
        ...
        {
            using (CheckAddrDialog dialog = new CheckAddrDialog())
            {
                dialog.ShowDialog();
            }
        }
        ...
        private void showAssetsToolStripMenuItem_Click(object sender, EventArgs e)
        {
            using (GetAssetInfo dialog = new GetAssetInfo())
            {
